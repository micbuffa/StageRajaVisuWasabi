...
define([], function () {
	return function NumericGlyph(id) {
		...
		var _selectedGlyph = id,     // �ndice do glifo criado
			...
			_xCenterCell = 0,
			_yCenterCell = 0,
			_glyphScale = d3.scale.linear(),

			_widthTooltip = 0,
			_heightTooltip = 0,
			_xCenterTooltip = 0,
			_yCenterTooltip = 0,
			_glyphScaleTooltip = d3.scale.linear(),

			_drawPolyline = d3.svg.line()
		...
		var _circleColor = "black",
			...
			_twinBarsYScale = d3.scale.ordinal();         // Escala em Y usada para desenhar barras
		...
		glyphStar.draw = function (cells) {
			...
			group.each(function (d, i) {
				...
				element.append("circle")
					...
					.style({ "fill": "none", "stroke": _starColor[0], "stroke-width": 0.2 });


				_vIndexMapAttr.forEach(function (d, i) {
					vData.push({
						x: _glyphScale(element.data()[0].values[_vIndexMapAttr[i]]) * Math.cos(i * angSector),
					...
					element.append("line")
						...
						.style({ "stroke": _starColor[0], "stroke-width": 0.2 });
		...
		glyphStar.drawTooltip = function (grpGlyphTooltip, cell) {
			...
			_vIndexMapAttr.forEach(function (d, i) {
				vData.push({
					x: _glyphScaleTooltip(cell.values[_vIndexMapAttr[i]]) * Math.cos(i * angSector),
				...
				if (angulo == 0) {
					...
					grpStarGlyph.append("text").text("  " + _data.edges[_vIndexMapAttr[i]])
						...
						.style({ "text-anchor": "start", "fill": "gray" });
				}
					...
					grpStarGlyph.append("text").text(_data.edges[_vIndexMapAttr[i]] + "  ")
						...
						.attr("class", "MG-Tooltip-Attr")
						.style({ "text-anchor": "end", "fill": "gray" });
				}
				else if (angulo == Math.PI) {
					grpStarGlyph.append("text").text(valor)
						.attr("x", xExtremo - 3).attr("y", yExtremo + 3)
			...
			grpStarGlyph.append("path")
				...
				.attr("fill", _starBkColor[0])
				.style({ "opacity": _starOpacity, "stroke": "black", "stroke-width": 1.5 });
		}
